...
Ext.define('Ext.ux.RowExpander', {
    ...
    onGridAfterLayout: function() {
        ...
        if (!grid.hasView) {
            ...
            if (this.expandOnDblClick && !this.expandOnClick) {
    ...
    collapseAll: function(){
    	var ln   = this.view.store.getCount(),
	        i = 0,
	        rowIdx;
	
	    for (; i < ln; i++) {
	        //rowIdx = this.view.store.indexOf(sels[i]);
	        this.toggleRow(i, true);
	    }
    },

    toggleRow: function(rowIdx, collapse) {
        var rowNode = this.view.getNode(rowIdx),
            row = Ext.get(rowNode),
            nextBd = Ext.get(row).down(this.rowBodyTrSelector),
            record = this.view.getRecord(rowNode);
        if(collapse && !this.recordsExpanded[record.internalId]) return;
        if (row.hasCls(this.rowCollapsedCls) && !collapse) {
