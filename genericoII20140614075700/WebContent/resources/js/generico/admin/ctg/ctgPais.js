CtgPais = function(){
	return {
		init : function(porcentajePuntuacion,titulo,cfg){
			var config = {};
			Ext.define('Writer.Grid', {
					...
					Ext.apply(this, {
						...
						columns: [
							...
							},
							{
								...
								editor: {
									id : 'ctgPaisPesoNacionalidadCombo',
									xtype: "combo",
			    ...
			    for(i = 0 ; i <= registros-1 ; i++) {
			    	...
			    	if(s.store.getAt(i).get("ctgPaisId") == Efx.constants.codes.COSTA_RICA
			    			&& puntajeCR != null){
			    		s.store.getAt(i).set("ctgPaisPesoNacionalidad",puntajeCR);
			    	}else{
			    	s.store.getAt(i).set("ctgPaisPesoNacionalidad",puntajeForAll);
			    	}
