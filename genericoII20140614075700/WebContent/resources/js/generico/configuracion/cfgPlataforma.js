...
//									{xtype:'label',text:title,cls: "ctg_label"}
						...
						columns: [
							...
							},
							{
								...
								minWidth: 300,
								sortable: true,
								dataIndex: 'cfgPlataformaDescripcion',
								field: {
									xtype: 'textfield',
									allowBlank: false
								},
				...
				onDeleteClick: function(){
					...
					if(sm.getSelection()[0]){
						...
						Efx.message.confirmDelete(function(p){
							var deleteOnServer = !Ext.isEmpty(p.sm.getSelection()[0].get("cfgPlataformaId"));
							p.sm.position.row = 0;
				...
				onAddClick: function(){
					var rec = new CfgPlataforma({
						cfgPlataformaNombre: '',
						cfgPlataformaActivo: true
					}), edit = this.editing;
					edit.cancelEdit();
			...
			Ext.define('CfgPlataforma', {
				extend: 'Ext.data.Model',
				idProperty: 'cfgPlataformaId',
				fields: [
				    {name: 'cfgPlataformaId'},
					{name: 'cfgPlataformaCodigo'},
					{name: 'cfgPlataformaDescripcion'}
				],
				validations:[
		            {type: 'presence',  field: 'cfgPlataformaDescripcion'}
	            ]
			});
			...
			var store = Ext.create('Ext.data.Store', {
				...
				proxy: {
					type: 'ajax',
					api: {
						read: Efx.constants.CONTEXT_PATH + '/cfgPlataforma/read/',
						create: Efx.constants.CONTEXT_PATH + '/cfgPlataforma/create/' ,
						update: Efx.constants.CONTEXT_PATH + '/cfgPlataforma/update/',
						destroy: Efx.constants.CONTEXT_PATH + '/cfgPlataforma/delete/'
					},
					reader: {
						type: 'json',
						totalProperty: 'total',
